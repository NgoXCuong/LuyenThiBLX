...
>>>>>>> Stashed changes
     ...
     * (đảm bảo tương thích với entity dùng int)
     */
    public int insertExamResultSync(ExamResult examResult) {
        try {
            Future<Long> future = executorService.submit(() -> examResultDao.insertExamResult(examResult));
            long id = future.get();
            return (int) id; // cast sang int
        } catch (Exception e) {
            e.printStackTrace();
            return -1;
        }
...
>>>>>>> Stashed changes
    ...
    /**
     * Lấy câu hỏi chi tiết bị sai (dùng cho màn làm lại câu sai)
     */
    ...
    /**
     * Phiên bản đồng bộ (nếu cần trong background thread)
     */
